...
def resume_training(model, optimizer, args, discriminator=False):
    ...
    if args.only_reward or args.cal_l1:
        return model, None, 1
    if discriminator:
...
def load_optim(model, optimizer, args):
    optim_path = os.path.join(args.load_path, "optimizer.pt")
    with FSDP.state_dict_type(
            model,
            StateDictType.FULL_STATE_DICT,
            FullStateDictConfig(offload_to_cpu=True, rank0_only=True),
