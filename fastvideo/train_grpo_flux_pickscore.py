    ...
    for index, batch_idx in enumerate(batch_indices):
        ...
        with torch.no_grad():
            for k in range(batch_size):
                ...
                # Instead, we use this one for pickscore
                if args.use_hpsv3:
                    with torch.amp.autocast('cuda'):
                        score = calc_probs(pick_processor, pick_reward_model, batch_caption[k], [image_path], device)
                        score = score.item()
                        hps_score = torch.tensor([score]).to(device)
                    all_rewards.append(hps_score)
                ...
                elif args.use_hpsv2 or args.use_hpsv3: # True
...
def main(args):
    ...
    if args.use_hpsv2: # False
        ...
        pick_reward_model = AutoModel.from_pretrained(model_pretrained_name_or_path).eval().to(device)
    ...
    if args.debug:
        ...
        args.name = 'debug'
        args.output_dir = './data/outputs_pickscore/debug/'
