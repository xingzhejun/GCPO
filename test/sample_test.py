...
class DualFluxPipeline(FluxPipeline):
    ...
    def __call__(
        ...
        mix_sampling_steps: int = 0, # modify from 10
...
def main(args):
    print('mix step is:', args.mix_sampling_steps)
    ...
    prompts = []
    with open(args.prompts_file, 'r', newline='', encoding='utf-8') as f:
        reader = csv.reader(f)
        next(reader) # header
        for row in reader:
            if row:  
    ...
    for prompt in prompts:
        ...
        safe_prompt = "".join(c if c.isalnum() or c in " _-" else "_" for c in prompt)[:200]
        image.save(os.path.join(args.output_dir, f"{id}_{safe_prompt}.jpg")) 
        print(f'saved {id}.jpg')
        id = id + 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompts_file", type=str, default="./test/test_prompts.csv",
    ...
    # modify
    parser.add_argument("--output_dir", type=str, default="./test/output_flux",
                        help="Directory to save generated images")
    parser.add_argument("--mix_sampling_steps", type=int, default=-1,
                        help="Number of sampling steps of the optimized model")
    parser.add_argument("--model_path", type=str,
                        help="Path to the optimized model checkpoint")
